...
pub struct UsersOAuth2PermissionGrant {
    pub client: Client,
}

impl UsersOAuth2PermissionGrant {
    #[doc(hidden)]
    pub fn new(client: Client) -> Self {
        UsersOAuth2PermissionGrant { client }
    }

    /**
    * List oauth2PermissionGrants.
    *
    * This function performs a `GET` to the `/users/{user-id}/oauth2PermissionGrants` endpoint.
    *
    ...
    *
    * FROM: <https://docs.microsoft.com/graph/api/user-list-oauth2permissiongrants?view=graph-rest-1.0>
    *
    * **Parameters:**
    *
    ...
    */
    pub async fn users_list_oauth_2_permission_grant(
        &self,
        user_id: &str,
        top: u64,
        skip: u64,
    ...
    *
    * This function performs a `GET` to the `/users/{user-id}/oauth2PermissionGrants/{oAuth2PermissionGrant-id}` endpoint.
    *
    * **Parameters:**
    *
    * * `select: &[String]` -- Select properties to be returned.
    * * `expand: &[String]` -- Expand related entities.
    */
    pub async fn users_get_oauth_2_permission_grant(
        &self,
        user_id: &str,
        o_auth_2_permission_grant_id: &str,
        select: &[String],
        expand: &[String],
    ) -> ClientResult<crate::types::MicrosoftGraphOAuth2PermissionGrantAllOf> {
        let mut query_args: Vec<(String, String)> = Default::default();
        if !expand.is_empty() {
            query_args.push(("$expand".to_string(), expand.join(" ")));
        }
        ...
        let url = self.client.url(
            &format!(
                "/users/{}/oauth2PermissionGrants/{}?{}",
                crate::progenitor_support::encode_path(&user_id.to_string()),
                crate::progenitor_support::encode_path(&o_auth_2_permission_grant_id.to_string()),
                query_
            ),
            None,
        );
    ...
    *
    * This function performs a `GET` to the `/users/{user-id}/oauth2PermissionGrants/$count` endpoint.
    *
    * **Parameters:**
    *
    * * `search: &str` -- The unique idenfier for an entity. Read-only.
    * * `filter: &str` -- The unique idenfier for an entity. Read-only.
    */
    pub async fn users_oauth_2_permission_grants_get_count_fb_7b(
        &self,
        user_id: &str,
        search: &str,
        filter: &str,
        ...
        let url = self.client.url(
            &format!(
                "/users/{}/oauth2PermissionGrants/$count?{}",
                crate::progenitor_support::encode_path(&user_id.to_string()),
                query_
            ),
            None,
